...
// Кулдаун между приступами ярости NYX-7 от обжорства. Раньше eatenCount
// сбрасывался в 0 сразу после ярости и NYX мог впасть в неё заново почти
// сразу же, стоило ему найти ещё 4 плохих еды — бесился слишком часто.
// Теперь после каждого приступа NYX не может взбеситься от еды снова,
// пока не пройдёт этот кулдаун (тикает в реальном времени, независимо
// от того, ест боссы или нет; отсчёт идёт от момента входа в ярость,
// поэтому реальный перерыв между концом одной ярости и началом
// следующей — это (кулдаун − длительность ярости)).
const NYX_FRENZY_COOLDOWN_TICKS = 550; // ~63 сек реального времени при тике 115мс — "минута, может больше"
...
function createBosses(GRID) {
  ...
  const defs = [
    ...
    },
    {
      ...
      randomChance: 0.28,
      // Радиус должен быть заметно больше chaseRange, иначе NYX почти
      // никогда не "видит" еду (она разбросана редко) и просто бегает
      // за игроками вместо того, чтобы питаться падалью
      eatRadius: 30,
      eatThreshold: 4,
      eatenCount: 0,
      // Ярость NYX-7 чуть дольше, чем у обычных боссов (140 тиков),
      // но реже — см. NYX_FRENZY_COOLDOWN_TICKS ниже.
      frenzyDuration: 165,
      frenzyCooldown: 0,
    ...
    },
    {
      ...
      teleportCooldownMax: 55,    // тиков между засадами
      warnDuration: 8,            // тиков предупреждения перед появлением
      materializedDuration: 9,    // тиков активной охоты после появления
...
// === Защита новых игроков от спавн-килла ===
// Пока у игрока не истёк bossInvulnerableUntil (заморозка спавна + пара
// секунд после того, как он реально начал двигаться), боссы полностью
// его игнорируют: не выбирают целью, не считают его для агро/фазы и не
// могут убить при столкновении. От стены и плохой еды это не защищает.
function isBossProtected(player) {
  return Boolean(player.bossInvulnerableUntil) && Date.now() < player.bossInvulnerableUntil;
}
...
function enrageBoss(boss, bosses, pushFeed, broadcast) {
  ...
  // Раньше эти счётчики тикали раз в несколько игровых тиков (пока боссы
  // двигались по общему троттлингу). Теперь фаза пересчитывается каждый
  // настоящий тик, поэтому длительности увеличены пропорционально, чтобы
  // ярость по ощущениям длилась примерно так же, как раньше (~18-20 сек).
  const duration   = boss.trait === "dash" ? 180 : 140;
  ...
  if (!wasEnraged) {
    pushFeed("boss", `👹 ${boss.name} в ЯРОСТИ!`, "", { key: "feed.bossRage", params: { boss: boss.name } });
    broadcast({ type: "notice", text: `⚠ ${boss.name} вошёл в ярость!` });
    for (const other of bosses) {
      if (other.id !== boss.id) other.agitatedTicks = Math.max(other.agitatedTicks || 0, 100);
    }
  }
}
...
function triggerNyxFrenzy(boss, bosses, pushFeed, broadcast) {
  const wasEnraged = boss.enragedTicks > 0;
  boss.enragedTicks = Math.max(boss.enragedTicks || 0, boss.frenzyDuration || 165);
  boss.size  = 2;
  boss.phase = "enraged";
  boss.angry = true;
  boss.eatenCount = 0;
  // Кулдаун стартует от момента входа в ярость, а не от её конца — так
  // суммарный перерыв между приступами гарантированно держится в районе
  // минуты, "может больше", а не сразу же после того как ярость спадёт.
  boss.frenzyCooldown = NYX_FRENZY_COOLDOWN_TICKS;

  if (!wasEnraged) {
    pushFeed("boss", `🤢 ${boss.name} объелась дряни и беснуется!`, "", { key: "feed.bossFrenzy", params: { boss: boss.name } });
...
function moveNyx(boss, bosses, alive, GRID, avoidCells, deps) {
  ...
  // В ярости NYX-7 еду не ищет и не ест — только гонится за змейкой.
  // Приоритет еды над погоней действует только вне ярости.
  const badFood = boss.phase === "enraged"
    ? null
    : findNearbyBadFood(deps.food, boss, boss.eatRadius || 30);
  ...
  if (boss.teleportCooldown <= 0 && head && dist <= (boss.teleportRange || 34)) {
    ...
    if (landing) {
      ...
      boss.gltchTimer = inRage
        ...
        : (boss.warnDuration || 5);
      if (wasWarning) deps.pushFeed?.("boss", `⚠ ${boss.name} готовит прыжок из ниоткуда!`, "", { key: "feed.bossTeleportWarning", params: { boss: boss.name } });
      return dist;
    }
  }
...
// === Скорость ===
// У каждого босса своя скорость, которая зависит ТОЛЬКО от его собственной
// фазы — ярость одного босса больше не разгоняет остальных. Скорость задана
// в клетках за игровой тик (дробная, копится в boss.moveAccum). Игрок
// перемещается примерно на 1 клетку за тик, поэтому:
//  - в ярости не-VØIDR боссы почти вплотную к скорости змейки;
//  - VØIDR в ярости чуть медленнее — он добирает недостающее рывками;
//  - вне ярости все заметно, но не критично медленнее, чем в ярости.
const BOSS_SPEED_FAST_DEFAULT = 0.92;
const BOSS_SPEED_SLOW_DEFAULT = 0.68;
const BOSS_SPEED_FAST_VOID    = 0.80;
const BOSS_SPEED_SLOW_VOID    = 0.55;

function bossSpeed(boss) {
  const hunting = boss.phase === "enraged"
    || (boss.trait === "phase" && boss.gltchState === "materialized");
  if (boss.trait === "dash") return hunting ? BOSS_SPEED_FAST_VOID : BOSS_SPEED_SLOW_VOID;
  return hunting ? BOSS_SPEED_FAST_DEFAULT : BOSS_SPEED_SLOW_DEFAULT;
}
...
function moveBosses({ bosses, players, food, tickCount: _tickCount, GRID, avoidCells, pushFeed, broadcast, killPlayer, pushFoodItem, createBadFood, removeFoodAt, insideGrid, pointKey }) {
  ...
  // Игроки, ещё защищённые после спавна, не видны боссам вообще —
  // ни как цель для погони, ни для расчёта агро/фазы.
  const targetable = alive.filter((p) => !isBossProtected(p));

  // Едоки (NYX-7) сначала "съедают" плохую еду под собой — это должно
  // случиться ДО общей зачистки ниже, иначе еда исчезает молча и
  // eatenCount/ярость от обжорства никогда не срабатывают.
  // В ярости NYX-7 еду больше не ест — это и есть "кулдаун" на обжорство:
  // пока бесится, гоняется только за змейкой, а не жрёт всё подряд без остановки.
  for (const boss of bosses) {
    if (boss.trait !== "eater") continue;
    // Кулдаун тикает в реальном времени каждый игровой тик, независимо от
    // фазы и от того, ест ли босс — это и есть перерыв между приступами
    // обжорной ярости NYX-7.
    if (boss.frenzyCooldown > 0) boss.frenzyCooldown -= 1;
    if (boss.phase === "enraged") continue;
    for (let i = food.length - 1; i >= 0; i--) {
      ...
      if (boss.eatenCount >= (boss.eatThreshold || 4) && (boss.frenzyCooldown || 0) <= 0) {
        triggerNyxFrenzy(boss, bosses, pushFeed, broadcast);
      }
    }
  }

  // Убрать оставшуюся еду под боссами (декоративная зачистка)
  for (let i = food.length - 1; i >= 0; i--) {
  ...
  for (const boss of bosses) {
    ...
    const nearestDist = targetable.length
      ? Math.min(...targetable.map((p) => distanceToBoss(p.snake[0], boss)))
      : 999;

    boss.angry = nearestDist <= BOSS_HUNT_RANGE || boss.phase === "enraged";
    updateBossPhase(boss, nearestDist);

    // Каждый босс копит собственную дробную скорость и действует только
    // когда накопил полный шаг — независимо от состояния других боссов.
    boss.moveAccum = (boss.moveAccum || 0) + bossSpeed(boss);
    if (boss.moveAccum >= 1) {
      boss.moveAccum -= 1;

      if (boss.trait === "dash")   moveVoidr(boss, bosses, targetable, GRID, avoidCells, deps);
      else if (boss.trait === "eater")  moveNyx(boss, bosses, targetable, GRID, avoidCells, deps);
      else if (boss.trait === "poison") moveScrap(boss, bosses, targetable, GRID, avoidCells, deps);
      else if (boss.trait === "phase")  moveGltch(boss, bosses, targetable, GRID, avoidCells, deps);
    }
  ...
  for (const player of alive) {
    if (isBossProtected(player)) continue;
    const head   = player.snake[0];
    const killer = bossAt(bosses, head);
    if (killer) {
      killPlayer(player, `${killer.name} схватил за голову`, {
        at: head, boss: killer,
        reasonKey: "death.grabbedByBoss", reasonParams: { boss: killer.name },
      });
      killer.moveCooldown = killer.phase === "enraged" ? 5 : 8;
    }
  }
}
