  ...
  const defs = [
    {
      ...
      x: 20, y: 20,
      // Берсерк-охотник: всегда гонится, рывки, быстро злится
    ...
    },
    {
      ...
      x: GRID.width - 24, y: 20,
      // Засадник: патрулирует углы, телепортируется, ставит ловушки
      chaseRange: BOSS_CHASE_RANGE - 4,
      randomChance: 0.35,    // много случайных ходов (обманывает)
      blinkRange: 12,        // дистанция прыжка (телепорт-шаг)
      blinkChance: 0.28,
      trapChance: 0.18,      // шанс оставить ловушку за ход
      patrolMode: "corners", // патрулирует углы карты
      patrolIdx: 0,
    },
    {
      id: "scrap", name: "SCR4P", color: "#ea580c", trait: "poison",
      x: Math.floor(GRID.width / 2) - 2, y: GRID.height - 24,
      // Территориальный: зигзаг, ядовитый след всегда, AOE при ярости
      chaseRange: BOSS_CHASE_RANGE,
      randomChance: 0.22,
      poisonAlways: true,    // яд даже без ярости
      poisonChance: 0.55,    // шанс оставить яд за шаг
      aoeRadius: 3,          // радиус AOE-яда при ярости
      zigzag: true,          // движется зигзагом
      zigzagDir: 1,
      zigzagTick: 0,
      patrolMode: "spiral",  // спиральный патруль
      spiralAngle: 0,
    },
  ];
  return defs.map((def) => ({
...
function applyBossStep(boss, move, avoidCells, GRID, deps) {
  ...
  // SCR4P оставляет яд всегда (с шансом), не только при ярости
  if (boss.trait === "poison") {
    const chance = boss.poisonAlways
      ? (boss.phase === "enraged" ? 0.8 : boss.poisonChance || 0.55)
      : (boss.phase === "enraged" ? 0.45 : 0);
    leavePoisonCell({ x: prevX, y: prevY }, avoidCells,
      { food, pushFoodItem, createBadFood, insideGrid, anyOccupies, pointKey }, chance);
  }
}

function leavePoisonCell(prev, avoidCells, { food, pushFoodItem, createBadFood, insideGrid, anyOccupies, pointKey }, chance = 0.45) {
...
// SCR4P: медленная расширяющаяся спираль от центра
function getSpiralTarget(boss, GRID, tickCount) {
  const cx = Math.floor(GRID.width / 2);
  const cy = Math.floor(GRID.height / 2);
  boss.spiralAngle = (boss.spiralAngle || 0) + 0.08;
  const r = 12 + (boss.spiralAngle / (Math.PI * 2)) * 5;
  return {
    x: Math.round(cx + Math.cos(boss.spiralAngle) * r),
    y: Math.round(cy + Math.sin(boss.spiralAngle) * r),
  };
}

// === Уникальная логика движения ===

// VØIDR — всегда преследует, рывки, легко злится
...
// NYX-7 — засадник: патрулирует углы, телепортируется, оставляет ловушки
function moveNyx(boss, bosses, alive, GRID, avoidCells, deps, tickCount) {
  const targetEntry = alive.reduce((best, p) => {
    const d = distanceToBoss(p.snake[0], boss);
    return !best || d < best.dist ? { player: p, dist: d } : best;
  }, null);
  ...
  const blinkRange = boss.blinkRange || 12;

  if (head && dist <= blinkRange) {
    // Режим засады — телепортируется (быстрое движение к жертве)
    const blinkSteps = boss.phase === "enraged" ? 4 : (Math.random() < (boss.blinkChance || 0.28) ? 3 : 1);
    for (let i = 0; i < blinkSteps; i++) {
      const m = pickDirectMove(bosses, boss, head, GRID);
      if (m) applyBossStep(boss, m, avoidCells, GRID, deps);
    }
  } else if (head && dist <= (boss.chaseRange || BOSS_CHASE_RANGE)) {
    // Средняя дистанция — обманное движение (иногда в сторону)
    if (Math.random() < (boss.randomChance || 0.35)) {
      const m = pickRandomLegal(bosses, boss, GRID);
      if (m) applyBossStep(boss, m, avoidCells, GRID, deps);
    } else {
      const m = pickDirectMove(bosses, boss, head, GRID);
  ...
  // Ловушки в случайных местах неподалёку
  if (Math.random() < (boss.trapChance || 0.18)) {
    const ox = boss.x + Math.floor(Math.random() * 7) - 3;
    const oy = boss.y + Math.floor(Math.random() * 7) - 3;
    leavePoisonCell({ x: ox, y: oy }, avoidCells, deps, 1.0);
  }

  return dist;
}

// SCR4P — территориальный: зигзаг + спираль, яд всегда
function moveScrap(boss, bosses, alive, GRID, avoidCells, deps, tickCount) {
  const targetEntry = alive.reduce((best, p) => {
    const d = distanceToBoss(p.snake[0], boss);
    return !best || d < best.dist ? { player: p, dist: d } : best;
  }, null);
  ...
  if (head && dist <= BOSS_HUNT_RANGE + 4) {
    ...
    // AOE яд при ярости — разбрасывает яд в радиусе
    if (boss.phase === "enraged") {
      const r = boss.aoeRadius || 3;
      for (let dy = -r; dy <= r; dy++) {
        for (let dx = -r; dx <= r; dx++) {
          if (Math.abs(dx) + Math.abs(dy) <= r && Math.random() < 0.35) {
    ...
    if (useZigzag) {
      ...
      // Берём перпендикулярный вектор к главному направлению
    ...
    // Патруль — спиральное движение по карте
    const spiral = getSpiralTarget(boss, GRID, tickCount);
...
function moveBosses({ bosses, players, food, tickCount, GRID, avoidCells, pushFeed, broadcast, killPlayer, pushFoodItem, createBadFood, insideGrid, pointKey }) {
  ...
  for (const boss of bosses) {
    ...
    if (boss.trait === "dash") {
      ...
      dist = moveNyx(boss, bosses, alive, GRID, avoidCells, deps, tickCount);
    } else if (boss.trait === "poison") {
      dist = moveScrap(boss, bosses, alive, GRID, avoidCells, deps, tickCount);
    }
