...
const challengesMod = require("./challenges");
...
const { DEFAULT_WHEEL } = require("../data/phrases");
...
function normalizeProfile(raw) {
  const entry = {
    ...
    equipped: {
      ...
      // Колесо чата (R → 1-4 в игре): ровно 4 слота, id фразы или null
      // (пустой слот). По умолчанию — 4 бесплатные фразы, см. data/phrases.js.
      phrases: normalizePhraseWheel(raw.equipped?.phrases),
    },
    stats: {
      ...
      // Прогресс ежедневных/недельных квестов (см. lib/challenges.js). Без
      // переноса сюда normalizeProfile на каждом чтении профиля обнулял бы
      // прогресс квестов, даже если он уже был честно сохранён в БД.
      challenges: raw.stats?.challenges && typeof raw.stats.challenges === "object" ? raw.stats.challenges : null,
  ...
  // Бесплатные фразы колеса чата (price 0) — доступны всем сразу, как и
  // default-скин, чтобы ownsItem/инвентарь были консистентны без спецкейсов.
  // !bundleOnly обязателен: у bundle-only фраз price:null, а Number(null)
  // === 0 в JS — без этого фильтра "six_seven" досталась бы всем бесплатно
  // в обход покупки коллаб-скина pepesnake.
  for (const phraseItem of SHOP_CATALOG.filter((i) => i.category === "phrase" && !i.bundleOnly && Number(i.price) === 0)) {
    if (!entry.inventory.includes(phraseItem.id)) entry.inventory.push(phraseItem.id);
  }
...
// Приводит сохранённое колесо чата к массиву ровно из 4 элементов
// (id фразы либо null). Невалидные/чужие id отбрасываются в null —
// реальная проверка владения всё равно происходит в lib/phrases.js
// и shop-actions.setPhraseWheel в момент использования/сохранения.
function normalizePhraseWheel(raw) {
  const src = Array.isArray(raw) ? raw : DEFAULT_WHEEL;
  return Array.from({ length: 4 }, (_, i) => src[i] || null);
}
...
function startNewLife(ctx, name) {
  ...
  challengesMod.applyGamePlayedProgress(entry);
  ctx.shopData[name] = entry;
  profileIndexSet(ctx, name);
