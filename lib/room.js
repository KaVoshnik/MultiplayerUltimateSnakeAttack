...
const { GRID, SPAWN_FREEZE_MS, SPAWN_BOSS_INVULN_MS, BONUS_TYPES, BONUS_LIFETIME_MS, COMBO_BRAG_MILESTONES, DEFAULT_TICK_MS: TICK_MS, MAX_PLAYERS } = gameConfig;
...
class Room {
  ...
  removePlayer(socketId) {
    ...
    if (this.players.size === 0) {
      if (this.started) {
        // Даём 10 секунд на переподключение (переход между страницами)
        setTimeout(() => {
          if (this.players.size === 0) this._destroy();
        }, 10000);
      } else {
  ...
  // игрока, получить его свежую позицию как полный join вместо
  // обычного mv. Нужно после респавна: _broadcastGameSync() использует
  // journal последнего тика, который не содержит информации о респавне
  // (он произошёл вне тика), поэтому без этого другие клиенты продолжали
  // бы хранить позицию игрока на месте смерти до следующего тика — и
  // увидели бы резкий "телепорт" через карту при следующем mv.
  _forceAoiResync(playerId) {
    for (const aoiSet of this.clientAoi.values()) aoiSet.delete(playerId);
  }
  ...
  _createPlayer(id, name, skin, cos) {
    ...
    const player = {
      ...
      best: this.bestForName ? this.bestForName(name) : 0, deaths: 0, reason: "",
      coinsEarned: 0, beatPersonalBest: false, sessionMvp: false,
      activeBonus: null, bonusExpires: null,
      combo: 0, maxCombo: 0,
      inventory: { ...prof.stats.foodInventory },
      // См. комментарий в lib/game-loop.js createPlayer — тот же счётчик,
      // отдельный от inventory (у него есть кап), для прогресса челленджей.
      lifeFoodCount: Object.fromEntries(Object.keys(prof.stats.foodInventory).map((k) => [k, 0])),
      avatar: cos.avatar,
  ...
  _extrasFor(p) {
    ...
    return {
      best: this.bestForName ? Math.max(p.best || 0, this.bestForName(p.name)) : (p.best || 0),
  ...
  _broadcastPresence() {
    ...
    if (this.getOnlineCount) snap.players = this.getOnlineCount();
    for (const id of this.players.keys()) this.send(id, snap);
  }
  ...
  _spawnBonus() {
    const spawn = bonusEffects.pickBonusSpawn({
      players: this.players, bonuses: this.bonuses, food: this.food,
      bosses: this.bosses, occupancySet: this.occupancySet,
      foodMod, bossMod, GRID, BONUS_TYPES,
    });
    if (!spawn) return;
    const { point, bonusType } = spawn;
    const expiresAt = Date.now() + BONUS_LIFETIME_MS;
    this.bonuses.push({ ...point, bonusType, spawnedAt: Date.now(), expiresAt });

    const j = gameSync.createJournal();
    j.bonusAdded.push(gameSync.compactBonus({ x: point.x, y: point.y, bonusType, expiresAt }, BONUS_TYPES));
    const ctx = this._syncCtx();
  ...
  _killPlayer(player, reason, opts = {}) {
    ...
    // См. комментарий в lib/game-loop.js killPlayer — reasonKey нужен
    // trackDeathStats ДО того, как он выставлен на player, для прогресса
    // челленджей ("Осторожный" — не умер от яда).
    const reasonKey = opts.killerPlayer ? "death.killedByPlayer" : (opts.reasonKey || null);
    if (this.trackDeathStats) this.trackDeathStats(player, { reasonKey });
    player.alive       = false;
    player.deaths     += 1;
    if (opts.killerPlayer) {
      player.reason       = `${opts.killerPlayer.name} убил тебя`;
      player.reasonKey    = reasonKey;
    ...
    if (this.recordScore) {
      try { this.recordScore(player); }
      catch (err) { console.error("Room recordScore error:", err.message); }
    }
    ...
    if (opts.boss) bossMod.enrageBoss(opts.boss, this.bosses,
      (kind, text, _playerName, meta) => {
        for (const id of this.players.keys()) {
          this.send(id, { type: "feed", feed: [{ kind, text, key: meta?.key || null, params: meta?.params || null, at: Date.now() }] });
        }
      },
  ...
  _tick() {
    ...
    // Тот же фикс, что и в основном server.js: раньше общий троттлинг
    // заставлял ярость ОДНОГО босса ускорять ВСЕХ. Теперь moveBosses
    // вызывается каждый тик, скорость каждого босса — независимая.
    bossMod.moveBosses({
      bosses: this.bosses, players: this.players, food: this.food,
      tickCount: this.tickCount, GRID, avoidCells: pathCells,
      pushFeed: (kind, text, _playerName, meta) => {
        for (const id of this.players.keys()) {
          this.send(id, { type: "feed", feed: [{ kind, text, key: meta?.key || null, params: meta?.params || null, at: Date.now() }] });
        }
      },
      ...
      removeFoodAt: (item) => {
        const idx = this.food.findIndex((f) => f.x === item.x && f.y === item.y);
        if (idx >= 0) {
          this.food.splice(idx, 1);
          this.tickJournal.foodRemoved.push([item.x, item.y]);
        }
      },
    ...
    bonusEffects.tickBonusEffects(this.players);
    bonusEffects.tickComboTimers(this.players);
    this._rebuildOccupancy();

    const { occupied, planned, targetCounts } = engine.planMoves(this.players, {
      GRID, skipPlayer: (p) => p.inLobby,
    });
    ...
    for (const player of this.players.values()) {
      ...
      if (!foodMod.insideGrid(nextHead, GRID)) {
        this._killPlayer(player, "Врезался в стену", { reasonKey: "death.wall" }); continue;
      }

      const resolvedKey = foodMod.pointKey(nextHead);
      if (targetCounts.get(key) > 1) { this._killPlayer(player, "Столкновение лоб в лоб", { reasonKey: "death.headOn" }); continue; }

      const killerBoss = bossMod.bossAt(this.bosses, nextHead);
      if (killerBoss && !bossMod.isBossProtected(player)) {
        this._killPlayer(player, `${killerBoss.name} поймал змейку`, {
      ...
      if (player.activeBonus !== "ghost" && occupied.has(resolvedKey)) {
        ...
        this._killPlayer(
          player,
          killer ? `${killer.name} убил` : "Столкнулся со змейкой",
      ...
      if (eaten) {
        ...
        if (eaten.good) {
          ...
          // Тот же таймер сгорания комбо, что и в публичном лобби (game-loop.js).
          player.comboExpires = Date.now() + bonusEffects.comboTimeoutMs(player.combo);
          ...
          if (player.combo === 5 || player.combo === 10) {
            for (const id of this.players.keys()) {
              this.send(id, { type: "feed", feed: [{
                kind: "combo", text: `🔥 ${player.name}: COMBO ×${player.combo}!`,
                key: "feed.combo", params: { name: player.name, combo: player.combo }, at: Date.now(),
              }] });
            }
          } else if (COMBO_BRAG_MILESTONES.includes(player.combo)) {
            for (const id of this.players.keys()) {
              this.send(id, { type: "feed", feed: [{
                kind: "combo", text: `👑 ${player.name}: КОМБО ×${player.combo}!!`,
                key: "feed.comboBrag", params: { name: player.name, combo: player.combo }, at: Date.now(),
              }] });
            }
          }
          ...
          if (player.lifeFoodCount[eaten.kind] !== undefined) player.lifeFoodCount[eaten.kind] += 1;
