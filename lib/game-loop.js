...
const { GRID, SPAWN_FREEZE_MS, SPAWN_BOSS_INVULN_MS, BONUS_TYPES, BONUS_LIFETIME_MS, COMBO_BRAG_MILESTONES, DEFAULT_TICK_MS } = gameConfig;
...
function createPlayer(ctx, id, name, skin) {
  ...
  const player = {
    ...
    // Счётчик съеденной хорошей еды за эту жизнь — отдельно от inventory
    // (у того есть кап INVENTORY_CAP, а прогресс челленджей не должен
    // на него натыкаться). Флашится в entry.stats.challenges при
    // смерти/дисконнекте, см. lib/stats-rewards.js.
    lifeFoodCount: Object.fromEntries(Object.keys(shopEntry.stats.foodInventory).map((k) => [k, 0])),
    avatar: cos.avatar, snakeHatEmoji: cos.snakeHatEmoji, snakeHatId: cos.snakeHatId,
...
function killPlayer(ctx, player, reason, opts = {}) {
  ...
  // Считаем reasonKey заранее (а не после trackDeathStats), чтобы
  // stats-rewards.trackDeathStats мог отличить смерть от яда для прогресса
  // челленджей ("Осторожный") — раньше reasonKey выставлялся только ПОСЛЕ
  // trackDeathStats и был там недоступен.
  const reasonKey = opts.killerPlayer ? "death.killedByPlayer" : (opts.reasonKey || null);
  statsRewards.trackDeathStats(ctx, player, { reasonKey });
  player.alive = false;
  player.deaths += 1;
...
function tick(ctx) {
  // Пишем ДО early-return по пустому лобби, иначе /health увидит растущий
  // tickLagMs просто из-за того, что на сервере сейчас никого нет, и
  // ошибочно решит, что тик завис.
  ...
  bonusEffects.tickComboTimers(ctx.players);
  ...
  for (const player of ctx.players.values()) {
    ...
    if (eaten) {
      ...
      if (eaten.good) {
        ...
        // Таймер сгорания комбо: чем выше комбо, тем меньше времени на
        // следующее яблоко (см. bonusEffects.comboTimeoutMs). Сбрасывается
        // (сгорает) в bonusEffects.tickComboTimers, вызывается ниже каждый тик.
        ...
        if (player.combo === 5 || player.combo === 10) {
          ...
          // Чисто хвастовская отметка — бонусов не даёт, только запись в
          // общей ленте событий.
          pushFeed(ctx, "combo", `👑 ${player.name}: КОМБО ×${player.combo}!!`, player.name,
            { key: "feed.comboBrag", params: { name: player.name, combo: player.combo } });
        }
