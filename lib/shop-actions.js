...
const { SHOP_CATALOG, SHOP_SKINS, AVATAR_PRESETS, getEffectivePrice, getPublicCatalog } = require("../data/shop-catalog");
...
function sendShopPayload(ctx, clientId, name) {
  ...
  ctx.send(clientId, {
    type: "shop_update", shopData: entry, skins: SHOP_SKINS, catalog: getPublicCatalog(), avatars: AVATAR_PRESETS,
    battlePass: getBattlePassConfig(), challenges: challengesMod.getChallengesPayload(entry),
...
// Выдаёт всё, что идёт "в комплекте" с товаром — сейчас это только
// bundledPhrase у коллаб-скинов (см. data/shop-catalog.js: pepesnake).
// Отдельная функция, а не инлайн, чтобы будущие бандлы (например, шляпа
// в комплекте со скином) добавлялись сюда одной строкой.
function grantBundledExtras(entry, item) {
  if (!item.bundledPhrase) return;
  const bundleItemId = `phrase_${item.bundledPhrase}`;
  if (!entry.inventory.includes(bundleItemId)) entry.inventory.push(bundleItemId);
}
