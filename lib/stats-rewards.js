...
function trackDeathStats(ctx, player, meta = {}) {
  ...
  if (player.lifeFoodCount) challengesMod.applyFoodProgress(entry, player.lifeFoodCount);
  challengesMod.applyGameFinishedProgress(entry, { score: player.score, diedFromPoison: meta.reasonKey === "death.ateBadFood" });
  if (player.lifeFoodCount) challengesMod.applyGlobalFoodProgress(ctx, player.lifeFoodCount);
  challengesMod.applyGlobalScoreProgress(ctx, player.score);
...
function awardKillCoins(ctx, killer, victim) {
  ...
  challengesMod.applyKillProgress(entry);
  challengesMod.applyGlobalKillProgress(ctx);
