...
const { BAD_FOOD_RATIO } = require("../config/game");

const FOOD_TARGET = 350;
const MIN_GOOD_FOOD = 150;
...
// Поле "weight" задаёт относительный шанс спавна хорошей еды — чем выше,
// тем чаще она встречается. Редкость нигде не подписывается игрокам явно,
// они считывают её сами по тому, как редко предмет пополняет инвентарь.
const FOOD_TYPES = {
  apple:      { good: true,  points: 6,  weight: 38, label: "яблоко" },
  cherry:     { good: true,  points: 7,  weight: 34, label: "вишню" },
  grape:      { good: true,  points: 6,  weight: 38, label: "виноград" },
  pineapple:  { good: true,  points: 12, weight: 11, label: "ананас" },
  coconut:    { good: true,  points: 20, weight: 4,  label: "кокос" },
...
const GOOD_FOOD_KINDS = ["apple", "cherry", "grape", "pineapple", "coconut"];
...
// Взвешенный выбор вида еды: более "редкие" виды имеют меньший weight
// и, соответственно, реже выпадают при спавне.
function pickWeightedKind(kinds) {
  const totalWeight = kinds.reduce((sum, k) => sum + (FOOD_TYPES[k].weight || 1), 0);
  let roll = Math.random() * totalWeight;
  for (const k of kinds) {
    roll -= FOOD_TYPES[k].weight || 1;
    if (roll <= 0) return k;
  }
...
function createGoodFood(point) {
  const kind = pickWeightedKind(GOOD_FOOD_KINDS);
...
function getAverageBadFoodRatio(players) {
  ...
  if (alive.length === 0) return BAD_FOOD_RATIO;
  const ratios = alive.map((p) => {
    // При высоком скоре снижаем долю яда — иначе карта забивается несъедобным
    const score = p.score || 0;
    const scaleback = Math.min(0.15, score / 20000); // до -15% при скоре 3000+
    return Math.max(0.1, BAD_FOOD_RATIO - scaleback);
  });
...
function getMovementPathCells(players, { GRID, tickCount }) {
  const cells = new Set();
  for (const player of players.values()) {
    if (!player.alive) continue;
    ...
    for (let i = 0; i < FOOD_PATH_AVOID_CELLS; i++) {
      x += dir.x;
      y += dir.y;
      // Стены всегда убивают — дальше по прямой считать смысла нет.
      if (!insideGrid({ x, y }, GRID)) break;
