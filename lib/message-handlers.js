...
function handleChallengeClaim(ctx, id, challengeId, nameHint) {
  const name = profiles.resolveName(ctx, id, nameHint);
  if (!name) return;
  const entry = profiles.getProfile(ctx, name);
  const result = challengesMod.claimChallenge(entry, challengeId);
  if (!result.ok) { ctx.send(id, { type: "notice", text: result.text }); return; }

  entry.coins = (Number(entry.coins) || 0) + result.reward;
  const player = ctx.players.get(id);
  if (player) player.coins = entry.coins;
  ctx.shopData[name] = entry;
  profiles.persistProfile(ctx, name, entry);

  ctx.send(id, { type: "notice", text: `${result.template.icon} ${result.template.name}: +${result.reward} монет!` });
  ctx.sendShopPayload(id, name);
  if (player) ctx.resyncPlayer(id);
}
...
async function handleJoin(ctx, id, message) {
  ...
  if (!resolved.ok) { ctx.send(id, { type: "notice", text: resolved.text }); return; }
  const name = resolved.name;
  // Досылаем реальный профиль (в т.ч. equipped.phrases) — иначе клиент
  // так и остаётся с дефолтным shopData из "hello" (там имя ещё не было
  // известно), и колесо чата в игре визуально показывает только
  // 4 базовые фразы, даже если в настройках выбраны другие.
  ctx.sendShopPayload(id, name);
  const prof = profiles.getProfile(ctx, name);
  const skin = profiles.getSkinDef(prof.activeSkin);
  profiles.startNewLife(ctx, name);
...
const prePlayerHandlers = {
  ...
  challenge_claim: (ctx, id, msg) => handleChallengeClaim(ctx, id, msg.challengeId, msg.name),
