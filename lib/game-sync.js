...
function compactBonus(item, bonusTypes) {
  ...
  return [item.x, item.y, item.bonusType, def?.color || "#fff", def?.label || "?", item.expiresAt || null];
}
...
function packPlayerFull(player, extras) {
  return {
    ...
    reasonKey: player.reasonKey || null,
    reasonParams: player.reasonParams || null,
  };
}
...
// Новые поля reasonKey/reasonParams добавлены В КОНЕЦ массива — это
// сохраняет обратную совместимость по индексам для всего, что было
// раньше (позиции 0-10 не менялись).
function packPlayerMeta(player, extras) {
  return [
    ...
    player.reason || "",
    player.reasonKey || null,
    player.reasonParams || null,
  ];
}
...
function buildDelta(ctx, clientId, journal) {
  ...
  // Base-поля: type, seq, tickMs — их всегда ровно 3. Дельту отбрасываем
  // только если сверх них НИЧЕГО не добавилось (иначе двигающиеся игроки
  // теряли бы обновление позиции почти на каждом тике: mv+pm — это уже
  // 5 ключей, что раньше тоже отбрасывалось порогом <= 5).
  if (Object.keys(delta).length <= 3) return null;
  return delta;
}
