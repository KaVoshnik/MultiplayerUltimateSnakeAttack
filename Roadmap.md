...
**Статус: ✅ база готова, механика применения — на потом (уже есть идеи).**

При поедании хорошей еды (яблоко/вишня/виноград) она теперь **дополнительно** копится в `player.inventory` по каждому виду отдельно, поверх существующего счёта — счёт и рост змейки не изменились ни на бит. Кап — 20 штук на вид (`config/game.js: INVENTORY_CAP`), обнуляется при каждой смерти/рестарте (инвентарь — часть объекта `player`, который пересоздаётся заново на respawn, отдельно ничего сбрасывать не нужно).

**Как это устроено:**
- `lib/food.js` экспортирует `GOOD_FOOD_KINDS` — единый источник видов еды, чтобы инвентарь не расходился с тем, что реально спавнится
- Инкремент происходит в обоих tick-loop'ах (`server.js` — публичное лобби, `lib/room.js` — приватные комнаты; они, как и раньше, дублируют друг друга — см. заметку про рефакторинг в исходном код-ревью)
- Синхронизация — **не** через общий AOI-broadcast (`pm`/`delta`), это личный ресурс игрока, который не нужен другим клиентам:
  - живые апдейты при поедании — отдельный message type `{ type: "inventory", inventory }`, шлётся только этому сокету
  - на (ре)спавне — подмешивается прямо в персональный `snapshot` (`sendSnapshot`/`_sendSnapshot` уже шлют пейлоад только одному клиенту, так что туда безопасно класть личные данные без нового канала)
- HUD: новый блок в `game.html` (`#inventoryHud`), рендерится как `🍎N 🍒N 🍇N` в `renderInventory()`

**Тест:** `test/room.test.js` — проверка, что `_createPlayer` инициализирует `{ apple: 0, cherry: 0, grape: 0 }`. Инкремент внутри tick-loop'ов не покрыт юнит-тестами — он завязан на полную тик-симуляцию (движение, коллизии, спавн еды), тестировать изолированно дорого; при появлении конкретной механики траты имеет смысл писать интеграционный тест сразу под неё.

**Осознанно не сделано — ждёт твоих идей:**
- [ ] Собственно трата инвентаря (крафт? способности? обмен между игроками? разблокировка чего-то в профиле?)
- [ ] Персистентность между жизнями/сессиями — сейчас инвентарь живёт только один заход, если станет мета-прогрессией (не просто ресурсом на один забег) — понадобится колонка в `players` и синк с БД по аналогии с `coins`
- [ ] UI для самого инвентаря сверх HUD-счётчика (если появится что-то, что нужно активно выбирать/использовать, а не просто копить)

**Файлы:** `config/game.js`, `lib/food.js`, `server.js`, `lib/room.js`, `public/game.html`, `public/js/game.js`, `test/room.test.js`
...
*Последнее обновление roadmap: июль 2026, часть 4 — добавлена база инвентаря еды (вне исходных 14 пунктов), готова под будущую механику применения.*
